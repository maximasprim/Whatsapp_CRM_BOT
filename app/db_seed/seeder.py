...
async def seed_roles(session: AsyncSession, permission_map: dict[str, Permission]) -> dict[str, Role]:
    """Roles are global (shared across every tenant), not tenant-scoped."""
...
async def seed_whatsapp_templates(session: AsyncSession, tenant_id: uuid.UUID) -> None:
    from sqlalchemy import select
    created = 0
    for tmpl_data in DEFAULT_WHATSAPP_TEMPLATES:
        ...
        if result.scalars().first():
            continue
        tmpl = WhatsAppTemplate(tenant_id=tenant_id, **tmpl_data)
        session.add(tmpl)
        created += 1
    await session.flush()
...
async def seed_default_tenant(session: AsyncSession) -> Tenant:
    """Create the default tenant that owns all existing/seeded data."""
    existing = await session.get(Tenant, uuid.UUID(DEFAULT_TENANT_ID))
    if existing:
    ...
    tenant = Tenant(
        ...
        name=os.getenv("DEFAULT_TENANT_NAME", "Default Business"),
        slug="default",
        is_active=True,
        plan="enterprise",
        ...
        whatsapp_webhook_verify_token=os.getenv("WHATSAPP_WEBHOOK_VERIFY_TOKEN", "SaiDika_crm_bot"),
...
async def seed_superuser(session: AsyncSession, role_map: dict[str, Role], tenant_id: uuid.UUID) -> User | None:
...
async def run_seed() -> None:
    ...
    async with AsyncSessionLocal() as session:
        try:
            ...
            await seed_whatsapp_templates(session, tenant_id=tenant.id)
            await seed_superuser(session, role_map, tenant_id=tenant.id)

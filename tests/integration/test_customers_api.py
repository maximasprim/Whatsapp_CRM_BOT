...
class TestCreateCustomer:
    async def test_creates_customer_and_returns_201(self, client: AsyncClient, auth_headers):
        resp = await client.post("/api/customers", json=VALID_CUSTOMER, headers=auth_headers)
    ...
    async def test_returns_401_without_authentication(self, client: AsyncClient):
        resp = await client.post("/api/customers", json=VALID_CUSTOMER)
        assert resp.status_code == 401

    async def test_returns_409_for_duplicate_phone(self, client: AsyncClient, auth_headers):
        await client.post("/api/customers", json=VALID_CUSTOMER, headers=auth_headers)
        resp2 = await client.post("/api/customers", json={**VALID_CUSTOMER, "email": "other@example.com"}, headers=auth_headers)
        assert resp2.status_code == 409

    async def test_returns_422_for_missing_required_fields(self, client: AsyncClient, auth_headers):
        resp = await client.post("/api/customers", json={"first_name": "Incomplete"}, headers=auth_headers)
...
class TestListCustomers:
    async def test_returns_paginated_list(self, client: AsyncClient, auth_headers):
        ...
        resp = await client.get("/api/customers?page=1&page_size=10", headers=auth_headers)
    ...
    async def test_search_filters_by_name(self, client: AsyncClient, auth_headers):
        ...
        resp = await client.get("/api/customers?search=UniqueSearchName", headers=auth_headers)
        assert resp.status_code == 200
        data = resp.json()
...
class TestGetCustomer:
    async def test_returns_customer_by_id(self, client: AsyncClient, auth_headers):
        create_resp = await client.post("/api/customers", json=VALID_CUSTOMER, headers=auth_headers)
        customer_id = create_resp.json()["id"]

        resp = await client.get(f"/api/customers/{customer_id}", headers=auth_headers)
        assert resp.status_code == 200
        assert resp.json()["id"] == customer_id

    async def test_returns_404_for_unknown_id(self, client: AsyncClient, auth_headers):
        import uuid
        resp = await client.get(f"/api/customers/{uuid.uuid4()}", headers=auth_headers)
...
class TestUpdateCustomer:
    async def test_updates_customer_fields(self, client: AsyncClient, auth_headers):
        ...
        resp = await client.put(f"/api/customers/{customer_id}", json={"city": "Mombasa"}, headers=auth_headers)
        assert resp.status_code == 200
...
class TestDeleteCustomer:
    async def test_deletes_customer_and_subsequent_get_returns_404(self, client: AsyncClient, auth_headers):
        ...
        del_resp = await client.delete(f"/api/customers/{customer_id}", headers=auth_headers)
